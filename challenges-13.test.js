...
const standardizePhoneNumbers = (phoneNumbers) => {
  ...
  for (let i = 0; i < phoneNumbers.length; i++) {
    let arr = phoneNumbers[i].split('');
    let filtered = arr.filter(item => RegExp(/[0-9]/).test(item)).join().replace(/,/g, '');
...
const allHappy = (strs) => {
  return strs.every( item => item.includes(':)'));
};
...
const findAnything = (strs, target) => {
  return strs.filter(item => item.includes(target));
};
...
const allAnything = (strs, target) => {
  return strs.every(item => item.includes(target));
};
...
const unenrollBrook = (roster) => {
  let newstring = [];
  roster.forEach( string => {
    newstring.push(string.filter(item => !item.includes('Brook')));
  });
  return newstring;

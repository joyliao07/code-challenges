...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++){
    arr.push(input[i].filter( num => typeof num === 'number' && num % 5 === 0));
  }

  for (let i = 0; i < arr.length; i++){
    answer.push(arr[i].map( num => Math.pow(2, num)));
  }
...
let findMaleAndFemale = (data) => {
  ...
  for (let i = 0; i < data.length; i++){
    arr = data.filter( name => name.gender === 'male' || name.gender === 'female');
  }
...
let findShortest = (data) => {
  let arr = data.map( character => character.name);

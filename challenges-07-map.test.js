...
const charCode = (input) => {
  let answer = [];
  let newArr = input.join().replace(/,/g, '');

  for (let i = 0; i < input.length; i++){
    answer.push(newArr.charCodeAt(i));
  }

  // newArr.map( function (newArr, i) {
  //   newArr.charCodeAt(i);
  // });
...
const extractAbilities = (abilities) => {
  return abilities.map (
    function (abilities, x) {
      return snorlaxAbilities.abilities[x].ability.name;
    }
  );
};
...
const extractStats = (stats) => {
  ...
  for (let i = 0; i < stats.length; i++){
    answer.push({});
    answer[i].name = stats[i].stat.name;
    answer[i].total = stats[i].effort + stats[i].baseStat;
  }

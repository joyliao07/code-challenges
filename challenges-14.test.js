...
const toTitleCase = (strs) => {
  ...
  for (let i = 0; i < strs.length; i++){
    ...
    for (let a = 0; a < wordArr.length; a++){
      if (a === 0) {
        wordNew.push(wordArr[0].toUpperCase());
      } else {
        wordNew.push(wordArr[a]);
      }
    }
    answer.push(wordNew.join().replace(/,/g, ''));
  }
  return answer;
};
...
let biggerThanLuke = (data) => {
  return data.filter(person => person.mass > 77).map( person => person.name).join().replace(/,/g,' - ');
...
const sortBy = (property, objs) => {
  if (property === 'price'){
    let answer = objs.sort( function (a, b){return a.price - b.price;} );
    return answer;
  } else if (property === 'name'){
    let answer2 = objs.sort((a, b) => a.name.localeCompare(b.name));
    return answer2;
  }
};

...
const countNumberOfElements = (input) => input.reduce( (acc, val) => acc += 1) ;
...
const countNumberOfChildren = (input) => {
  let arr = input.filter( house => house.children);
...
const extractStat = (statName, input) => {
  let arr = input.filter( f => f.stat.name === statName);
  return arr.reduce((acc, val) =>  acc.push(val));
...
const reversedString = (input) => {
  let reverse = input.split('').reduce((acc,val) => {
    acc.unshift(val);
    return acc;
  }, []);
  return reverse.join().replace(/,/g, '');
};
